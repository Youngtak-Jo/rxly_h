            ...
            <main className="pt-32 pb-24 px-6 md:px-10 max-w-5xl mx-auto">
                <header className="mb-16 max-w-2xl">
                    <div className="inline-flex items-center gap-2 px-3 py-1 mb-6 rounded-full bg-blue-50 border border-blue-100/50">
                        ...
                        <span className="text-xs font-semibold uppercase tracking-wider text-blue-800">{t("badge")}</span>
                    </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 mb-20">
                    <div className="bg-white p-6 rounded-2xl border border-black/[0.08] shadow-sm">
                        ...
                        <h3 className="font-semibold text-black mb-2">{t("cards.search.title")}</h3>
                        <p className="text-sm text-gray-500 leading-relaxed">{t("cards.search.description")}</p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl border border-black/[0.08] shadow-sm">
                        <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center mb-4">
                            <Zap className="w-5 h-5 text-blue-600" />
                        </div>
                        <h3 className="font-semibold text-black mb-2">{t("cards.parallel.title")}</h3>
                        <p className="text-sm text-gray-500 leading-relaxed">{t("cards.parallel.description")}</p>
                    </div>
                    <div className="bg-white p-6 rounded-2xl border border-black/[0.08] shadow-sm">
                        <div className="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center mb-4">
                            <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                        </div>
                        <h3 className="font-semibold text-black mb-2">{t("cards.citations.title")}</h3>
                        <p className="text-sm text-gray-500 leading-relaxed">{t("cards.citations.description")}</p>
                    </div>
                </div>

                {/* Roster List */}
                <section>
                    <h2 className="text-2xl font-serif text-black mb-8 border-b border-black/[0.08] pb-4">{t("sectionTitle")}</h2>

...
function validateDbHealthAccess(req: Request): NextResponse | null {
    const adminKey = process.env.DB_HEALTH_ADMIN_KEY
    if (!adminKey) {
        return NextResponse.json(
            { status: "disabled", error: "DB health endpoint is disabled" },
            { status: 503 }
        )
    }

    const providedKey = req.headers.get("x-db-health-key")
...
export async function GET(req: Request) {
    ...
    try {
        await requireAuth()
        const denied = validateDbHealthAccess(req)
...
export async function POST(req: Request) {
    try {
        ...
        if (process.env.DB_HEALTH_ALLOW_TERMINATE !== "true") {
            return NextResponse.json(
                { error: "Termination is disabled" },
                { status: 403 }
            )
        }

        // Terminate only connections owned by our user (not superuser)
        const terminated = await prisma.$queryRaw<{ terminated: boolean; pid: number }[]>`
        ...
        await prisma.$queryRaw`SET idle_in_transaction_session_timeout = '300000'`

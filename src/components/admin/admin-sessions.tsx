...
export function AdminSessions() {
  ...
  const { data, error, isLoading, isRefreshing } = useAdminQuery<AdminSessionsResponse>({
    cacheKey: `admin:sessions:${baseQuery}`,
    url: `/api/admin/sessions?${baseQuery}`,
    refreshToken,
  })
  const { data: previousData } = useAdminQuery<AdminSessionsResponse>({
    cacheKey: `admin:sessions:previous:${previousQuery}`,
    url: `/api/admin/sessions?${previousQuery}`,
    refreshToken,
  })
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const response = await adminFetchJson<AdminSessionsResponse>({
        cacheKey: `admin:sessions:${query}`,
        url: `/api/admin/sessions?${query}`,
        force: true,
      })
